...
export function LightHouse() {
  ...
  if (selected) {
    ...
    return (
      <div style={{ width: "100%", minHeight: "100vh", background: "var(--comic-bg, #0F1117)", fontFamily: "'Inter', system-ui, sans-serif", color: "var(--comic-text-primary, #E8EAF0)", display: "flex", flexDirection: "column" }}>
        <div style={{ height: 56, borderBottom: `1px solid ${COLOR}25`, display: "flex", alignItems: "center", padding: "0 24px", gap: 16, background: "var(--comic-surface, #0D0F14)" }}>
          <button onClick={() => setSelected(null)} style={{ color: COLOR, background: "none", border: "none", cursor: "pointer", fontSize: 14 }}>← Back</button>
          <div style={{ flex: 1, fontSize: 16, fontWeight: 700, color: "var(--comic-text-primary, #F9FAFB)" }}>🏠 Listing Detail</div>
        </div>
        <div style={{ flex: 1, padding: "32px 40px", overflow: "auto" }}>
          ...
          <div style={{ display: "flex", gap: 24 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 24, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 8 }}>{l.title}</div>
              ...
              <div style={{ padding: "16px", borderRadius: 12, background: `${COLOR}08`, border: `1px solid ${COLOR}20` }}>
                ...
                <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)", lineHeight: 1.6 }}>Your exact location is never shown to hosts until you confirm a booking. All communications are encrypted.</div>
              </div>
            </div>
            <div style={{ width: 280, flexShrink: 0 }}>
              <div style={{ padding: "24px", borderRadius: 16, background: "rgba(255,255,255,0.03)", border: `1px solid ${COLOR}25` }}>
                <div style={{ fontSize: 32, fontWeight: 800, color: COLOR, marginBottom: 4 }}>${l.price}<span style={{ fontSize: 14, color: "var(--comic-text-secondary, #6B7280)", fontWeight: 400 }}>/mo</span></div>
                {l.credits && <div style={{ fontSize: 12, color: "#F59E0B", marginBottom: 16 }}>✓ Accepts Service Credits</div>}
                <button style={{ width: "100%", padding: "12px", borderRadius: 10, background: COLOR, border: "none", color: "var(--comic-bg, #0F1117)", fontWeight: 800, fontSize: 15, cursor: "pointer", marginBottom: 10 }}>Apply Now</button>
    ...
    <div style={{ width: "100%", height: "100%", minHeight: "100vh", background: "var(--comic-bg, #0F1117)", fontFamily: "'Inter', system-ui, sans-serif", color: "var(--comic-text-primary, #E8EAF0)", display: "flex" }}>
      ...
      <aside style={{ width: 240, background: "var(--comic-surface, #0D0F14)", borderRight: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "20px 16px 12px" }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", color: "var(--comic-text-secondary, #6B7280)", textTransform: "uppercase", marginBottom: 12 }}>🏠 LightHouse</div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <header style={{ height: 56, borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", padding: "0 24px", gap: 16, background: "var(--comic-surface, #0D0F14)", flexShrink: 0 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: "var(--comic-text-primary, #E8EAF0)" }}>🏠 LightHouse — Safe Housing</div>
            <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)" }}>1,204 verified listings · Privacy-first </div>
          </div>
        ...
        {tab === "browse" ? (
                ...
                {LISTINGS.map((l) => (
                  <div key={l.id} style={{ borderRadius: 16, background: "rgba(255,255,255,0.02)", border: `1px solid ${COLOR}20`, overflow: "hidden", cursor: "pointer" }}>
                    ...
                    <div style={{ padding: "16px" }}>
                      ...
                      <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)", marginBottom: 8 }}><MapPin size={11} style={{ display: "inline", verticalAlign: "middle" }} /> {l.location}</div>
          ...
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12, color: "#4B5563" }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: "var(--comic-text-secondary, #6B7280)" }}>Saved Listings</div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ padding: "8px 24px 20px", flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14 }}>
                ...
                <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="Describe your housing needs — budget, location, safety…" style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontSize: 14, color: "var(--comic-text-primary, #E8EAF0)" }} />
      ...
      <aside style={{ width: 280, borderLeft: "1px solid rgba(255,255,255,0.06)", background: "var(--comic-surface, #0D0F14)", padding: "20px 16px", flexShrink: 0 }}>
        ...
        {[{ range: "$500–800", l: "Emergency/Studio", c: "#22C55E" }, { range: "$800–1,200", l: "1 Bedroom", c: COLOR }, { range: "$1,200+", l: "2+ Bedrooms", c: "var(--comic-text-secondary, #6B7280)" }].map(({ range, l, c }) => (
        ...
        <div style={{ marginTop: 16, padding: "14px 16px", borderRadius: 12, background: `${COLOR}08`, border: `1px solid ${COLOR}20` }}>
          ...
          <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)", lineHeight: 1.6 }}>Your location is never exposed to landlords without your consent. All matches are encrypted.</div>
        </div>
      </aside>
    </div>
  );
