    ...
    <div style={{ width: "100%", height: "100%", minHeight: "100vh", background: "var(--comic-bg, #0F1117)", fontFamily: "'Inter', system-ui, sans-serif", color: "var(--comic-text-primary, #E8EAF0)", display: "flex" }}>
      ...
      <aside style={{ width: 240, background: "var(--comic-surface, #0D0F14)", borderRight: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "20px 16px 12px" }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", color: "var(--comic-text-secondary, #6B7280)", textTransform: "uppercase", marginBottom: 12 }}>😁 Mood</div>
          <div style={{ padding: "14px 16px", borderRadius: 12, background: `${COLOR}08`, border: `1px solid ${COLOR}20` }}>
            ...
            <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)", lineHeight: 1.6 }}>Your mood is never linked to your identity. Zero tracking. Encrypted.</div>
          </div>
        </div>
        <ScrollArea style={{ flex: 1 }}>
          <div style={{ padding: "0 8px 16px" }}>
            ...
            <div style={{ padding: "8px 10px", fontSize: 12, color: "var(--comic-text-secondary, #6B7280)" }}>Check-ins today: <span style={{ color: COLOR, fontWeight: 600 }}>12,847</span></div>
            <div style={{ padding: "8px 10px", fontSize: 12, color: "var(--comic-text-secondary, #6B7280)" }}>Streak: <span style={{ color: COLOR, fontWeight: 600 }}>7 days 🔥</span></div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <header style={{ height: 56, borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", padding: "0 24px", gap: 16, background: "var(--comic-surface, #0D0F14)", flexShrink: 0 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: "var(--comic-text-primary, #E8EAF0)" }}>😁 Mood — Anonymous Check-ins</div>
            <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)" }}>Zero tracking · Community wellness </div>
          </div>
        ...
        {tab === "checkin" ? (
              ...
              {!submitted ? (
                <>
                  <div style={{ textAlign: "center", marginBottom: 40 }}>
                    <div style={{ fontSize: 28, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 8 }}>How are you feeling right now?</div>
                    <div style={{ fontSize: 15, color: "var(--comic-text-secondary, #6B7280)" }}>Anonymous, safe, and completely private. Your mood never leaves this device.</div>
                  </div>
                  <div style={{ display: "flex", gap: 16, justifyContent: "center", marginBottom: 40 }}>
                    {MOODS.map((m) => (
                      <button key={m.value} onClick={() => setSelected(m.value)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "20px 16px", borderRadius: 16, background: selected === m.value ? `${m.color}20` : "rgba(255,255,255,0.02)", border: `2px solid ${selected === m.value ? m.color : "rgba(255,255,255,0.06)"}`, cursor: "pointer", transition: "all 0.15s" }}>
                        ...
                        <div style={{ fontSize: 12, fontWeight: 600, color: selected === m.value ? m.color : "var(--comic-text-secondary, #6B7280)" }}>{m.label}</div>
                      </button>
                    ))}
                  </div>
                  {selected && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                      <textarea placeholder="(Optional) Anything you'd like to add? Completely anonymous…" rows={3} style={{ width: "100%", padding: "14px 16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, fontSize: 14, color: "var(--comic-text-primary, #E8EAF0)", outline: "none", resize: "none", boxSizing: "border-box" }} />
                ...
                <div style={{ textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 28, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 8 }}>Thank you for checking in.</div>
                  <div style={{ fontSize: 15, color: "var(--comic-text-secondary, #6B7280)", marginBottom: 32 }}>You're part of a community of 4.9M survivors supporting each other.</div>
                    ...
                    {RESOURCES.map((r) => (
                      <div key={r.title} style={{ padding: "14px 18px", borderRadius: 12, background: `${r.color}10`, border: `1px solid ${r.color}30`, display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }}>
                        ...
                        <div style={{ flex: 1, textAlign: "left" }}>
                          <div style={{ fontSize: 14, fontWeight: 600, color: "var(--comic-text-primary, #E8EAF0)" }}>{r.title}</div>
          ...
          <ScrollArea style={{ flex: 1 }}>
            <div style={{ padding: "24px" }}>
              <div style={{ fontSize: 22, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 4 }}>Community Wellness Trends</div>
              <div style={{ fontSize: 14, color: "var(--comic-text-secondary, #6B7280)", marginBottom: 24 }}>Aggregated anonymous data · Individual data never exposed</div>
                ...
                <div style={{ display: "flex", gap: 12, alignItems: "flex-end", height: 120 }}>
                  {COMMUNITY_MOOD.map((d) => (
                    <div key={d.day} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                      ...
                      <div style={{ fontSize: 11, color: "var(--comic-text-secondary, #6B7280)" }}>{d.day}</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
                {[{ l: "Daily Check-ins", v: "12,847", delta: "+4.2%", c: "#22C55E" }, { l: "Avg Mood Score", v: "4.1/5", delta: "+0.3 this week", c: COLOR }, { l: "Crisis Connections", v: "23", delta: "Handled safely", c: "#EF4444" }].map(({ l, v, delta, c }) => (
                  <div key={l} style={{ padding: "20px", borderRadius: 14, background: `${c}08`, border: `1px solid ${c}20` }}>
                    ...
                    <div style={{ fontSize: 13, color: "var(--comic-text-primary, #F9FAFB)", fontWeight: 600, marginBottom: 4 }}>{l}</div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ padding: "8px 24px 20px", flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14 }}>
                ...
                <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="How are you feeling? (Always anonymous)" style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontSize: 14, color: "var(--comic-text-primary, #E8EAF0)" }} />
      ...
      <aside style={{ width: 280, borderLeft: "1px solid rgba(255,255,255,0.06)", background: "var(--comic-surface, #0D0F14)", padding: "20px 16px", flexShrink: 0 }}>
        ...
        <div style={{ padding: "20px", borderRadius: 14, background: `${COLOR}08`, border: `1px solid ${COLOR}20`, textAlign: "center", marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 13, color: "var(--comic-text-secondary, #6B7280)" }}>Community average today</div>
        </div>
