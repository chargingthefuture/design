...
export function Directory({
  ...
  if (selected && selectedProfile) {
    ...
    return (
      <div style={{ width: "100%", height: "100%", minHeight: "100vh", background: "var(--comic-bg, #0F1117)", fontFamily: "'Inter', system-ui, sans-serif", color: "var(--comic-text-primary, #E8EAF0)", display: "flex", flexDirection: "column" }}>
        <div style={{ height: 56, borderBottom: `1px solid ${COLOR}25`, display: "flex", alignItems: "center", padding: "0 24px", gap: 16, background: "var(--comic-surface, #0D0F14)", flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, fontSize: 16, fontWeight: 700, color: "var(--comic-text-primary, #F9FAFB)" }}>📇 Provider Profile</div>
        </div>
            ...
            <div style={{ display: "flex", gap: 24, marginBottom: 28 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                  <div style={{ fontSize: 24, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)" }}>{fullName}</div>
                  ...
                  <>
                    <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                      <input
                        ...
                        style={{ flex: 1, padding: "9px 12px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(99,102,241,0.28)", borderRadius: 8, fontSize: 13, color: "var(--comic-text-primary, #E8EAF0)", outline: "none", fontFamily: "monospace" }}
                      />
                      <button
                ...
                {[
                  ...
                  <div key={i} style={{ padding: "16px", borderRadius: 12, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", marginBottom: 10 }}>
                    <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "var(--comic-text-primary, #E8EAF0)" }}>{r.reviewer}</div>
              ...
              <div>
                <div style={{ padding: "20px", borderRadius: 16, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", marginBottom: 16 }}>
                  ...
                  {["Mon – Fri", "10:00 AM – 6:00 PM", "Accepts Service Credits ✓"].map(line => (
                    <div key={line} style={{ fontSize: 13, color: "var(--comic-text-primary, #E8EAF0)", marginBottom: 6 }}>{line}</div>
                  ))}
                </div>
                <div style={{ padding: "20px", borderRadius: 16, background: `${COLOR}08`, border: `1px solid ${COLOR}20` }}>
                  ...
                  <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)", lineHeight: 1.6 }}>All messages are encrypted and trauma-informed by design.</div>
                </div>
              </div>
            </div>
          </div>
    ...
    <div style={{ width: "100%", height: "100%", minHeight: "100vh", background: "var(--comic-bg, #0F1117)", fontFamily: "'Inter', system-ui, sans-serif", color: "var(--comic-text-primary, #E8EAF0)", display: "flex" }}>
      ...
      <aside style={{ width: 240, background: "var(--comic-surface, #0D0F14)", borderRight: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "20px 16px 12px" }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", color: "var(--comic-text-secondary, #6B7280)", textTransform: "uppercase", marginBottom: 12 }}>📇 Directory</div>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {["All", "Verified", "Online", "Credits"].map(f => (
              <button key={f} onClick={() => setActiveFilter(f)} style={{ padding: "4px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, background: activeFilter === f ? `${COLOR}20` : "rgba(255,255,255,0.04)", border: `1px solid ${activeFilter === f ? COLOR + "50" : "rgba(255,255,255,0.06)"}`, color: activeFilter === f ? COLOR : "var(--comic-text-secondary, #6B7280)", cursor: "pointer" }}>{f}</button>
            ))}
          </div>
        </div>
          ...
          <div style={{ padding: "0 8px 16px" }}>
            {FILTERS.map(f => (
              <div key={f} onClick={() => setActiveFilter(f)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8, cursor: "pointer", background: activeFilter === f ? `${COLOR}18` : "transparent", borderLeft: activeFilter === f ? `2px solid ${COLOR}` : "2px solid transparent", marginLeft: 2 }}>
                <span style={{ fontSize: 13, color: activeFilter === f ? "var(--comic-text-primary, #E8EAF0)" : "#9CA3AF", flex: 1 }}>{f}</span>
              </div>
            ))}
        ...
        <div style={{ padding: 12, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ padding: "10px 12px", borderRadius: 10, background: `${COLOR}10`, border: `1px solid ${COLOR}25` }}>
            ...
            <div style={{ fontSize: 11, color: "var(--comic-text-secondary, #6B7280)" }}>Claim your profile today</div>
          </div>
        </div>
      </aside>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <header style={{ height: 56, borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", padding: "0 24px", gap: 16, background: "var(--comic-surface, #0D0F14)", flexShrink: 0 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: "var(--comic-text-primary, #E8EAF0)" }}>📇 Directory</div>
            <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)" }}>47,234 verified providers · Trauma-informed · Safe</div>
          </div>
          ...
          <button onClick={toggleEmpty} style={{ padding: "4px 12px", borderRadius: 20, background: emptyMode ? "#EF444420" : "rgba(255,255,255,0.04)", border: emptyMode ? "1px solid #EF444440" : "1px solid rgba(255,255,255,0.08)", color: emptyMode ? "#EF4444" : "var(--comic-text-secondary, #6B7280)", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
          ...
          emptyMode ? (
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", gap: 16 }}>
              ...
              <div style={{ textAlign: "center", maxWidth: 400 }}>
                <div style={{ fontSize: 20, fontWeight: 700, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 8 }}>
                  {genuinelyEmpty && !toggledEmpty ? "No providers listed yet" : "No providers found"}
                </div>
                <div style={{ fontSize: 14, color: "var(--comic-text-secondary, #6B7280)", lineHeight: 1.7, marginBottom: showBrowseAll ? 24 : 0 }}>
            ...
            <ScrollArea style={{ flex: 1 }}>
              <div style={{ padding: "24px" }}>
                <div style={{ marginBottom: 20, padding: "20px 24px", borderRadius: 16, background: `linear-gradient(135deg,${COLOR}20 0%,rgba(14,165,233,0.1) 100%)`, border: `1px solid ${COLOR}25` }}>
                  <div style={{ fontSize: 22, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 4 }}>Find Your Support Network</div>
                      ...
                      <div style={{ display: "flex", gap: 14, marginBottom: 14, alignItems: "flex-start" }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                            <div style={{ fontSize: 15, fontWeight: 700, color: "var(--comic-text-primary, #F9FAFB)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{p.firstName} {p.lastName}</div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
            ...
            <div style={{ padding: "8px 24px 20px", flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14 }}>
                ...
                <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder="Find providers, ask questions…" style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontSize: 14, color: "var(--comic-text-primary, #E8EAF0)" }} />
      ...
      {/* Right panel */}
      <aside style={{ width: 280, borderLeft: "1px solid rgba(255,255,255,0.06)", background: "var(--comic-surface, #0D0F14)", display: "flex", flexDirection: "column", padding: "20px 16px", flexShrink: 0 }}>
        ...
        {PROFILES.filter(p => p.online).slice(0, 4).map(p => (
          <div key={p.id} onClick={() => setSelected(p.id)} style={{ display: "flex", gap: 10, alignItems: "center", padding: "10px", borderRadius: 10, background: "rgba(255,255,255,0.02)", border: `1px solid ${COLOR}15`, marginBottom: 8, cursor: "pointer" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--comic-text-primary, #E8EAF0)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{p.firstName} {p.lastName}</div>
              <div style={{ fontSize: 11, color: "var(--comic-text-secondary, #6B7280)" }}>{p.role}</div>
            </div>
        ...
        <div style={{ marginTop: 20, padding: "16px", borderRadius: 12, background: `${COLOR}08`, border: `1px solid ${COLOR}20` }}>
          ...
          <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)", lineHeight: 1.6 }}>Your identity is protected. All interactions are encrypted.</div>
        </div>
