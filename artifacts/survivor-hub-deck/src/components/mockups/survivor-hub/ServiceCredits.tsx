...
export function ServiceCredits({ initialEmpty = false }: { initialEmpty?: boolean } = {}) {
  ...
  const typeColor = (type: string) => {
    switch (type) {
      ...
      default: return "var(--comic-text-secondary, #6B7280)";
    }
  };
    ...
    <div style={{ width: "100%", height: "100%", minHeight: "100vh", background: "var(--comic-bg, #0F1117)", fontFamily: "'Inter', system-ui, sans-serif", color: "var(--comic-text-primary, #E8EAF0)", display: "flex" }}>
      ...
      <aside style={{ width: 240, background: "var(--comic-surface, #0D0F14)", borderRight: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "20px 16px 12px" }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", color: "var(--comic-text-secondary, #6B7280)", textTransform: "uppercase", marginBottom: 12 }}>⚙️ Service Credits</div>
        </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <header style={{ height: 56, borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", padding: "0 24px", gap: 16, background: "var(--comic-surface, #0D0F14)", flexShrink: 0 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: "var(--comic-text-primary, #E8EAF0)" }}>⚙️ ServiceCredits — Utility Tokens</div>
            <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)" }}>Earn · Spend · Trade · Usable across all 18 plugins · Phase 3</div>
          </div>
        ...
        {tab === "wallet" ? (
          emptyMode ? (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", gap: 20 }}>
              ...
              <div style={{ textAlign: "center", maxWidth: 380 }}>
                <div style={{ fontSize: 20, fontWeight: 700, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 8 }}>Your wallet is empty</div>
                <div style={{ fontSize: 14, color: "var(--comic-text-secondary, #6B7280)", lineHeight: 1.7 }}>Earn your first ServiceCredits by completing a Skills Hunt round, verifying your profile, or receiving a peer transfer. Every transaction is recorded on the Formance ledger.</div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10, width: "100%", maxWidth: 440 }}>
                {[
                  ...
                  <div key={action} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", borderRadius: 12, background: `${color}08`, border: `1px solid ${color}20` }}>
                    ...
                    <span style={{ flex: 1, fontSize: 13, color: "var(--comic-text-primary, #E8EAF0)" }}>{action}</span>
              ...
              <button style={{ padding: "12px 28px", borderRadius: 12, background: COLOR, border: "none", color: "var(--comic-bg, #0F1117)", fontSize: 14, fontWeight: 800, cursor: "pointer" }}>
            ...
            <div style={{ padding: "24px" }}>
              ...
              <div style={{ marginBottom: 24, padding: "28px 32px", borderRadius: 20, background: `linear-gradient(135deg,${COLOR}25 0%,rgba(245,158,11,0.05) 100%)`, border: `1px solid ${COLOR}30` }}>
                ...
                <div style={{ display: "flex", gap: 12 }}>
                  <button style={{ flex: 1, padding: "12px", borderRadius: 12, background: COLOR, border: "none", color: "var(--comic-bg, #0F1117)", fontSize: 14, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}><ArrowUp size={16} /> Send</button>
              ...
              <div style={{ padding: "20px 24px", borderRadius: 16, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                <div style={{ fontSize: 16, fontWeight: 700, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 16 }}>Recent Transactions</div>
                {TRANSACTIONS.map((t) => (
                  <div key={t.id} style={{ display: "flex", gap: 14, alignItems: "center", padding: "12px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "var(--comic-text-primary, #E8EAF0)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{t.description}</div>
          ...
          <ScrollArea style={{ flex: 1 }}>
            <div style={{ padding: "24px" }}>
              <div style={{ fontSize: 22, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 4 }}>Earn Service Credits</div>
              <div style={{ fontSize: 14, color: "var(--comic-text-secondary, #6B7280)", marginBottom: 20 }}>Contribute to the community and get rewarded</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 28 }}>
                {EARN_METHODS.map((m) => (
                  <div key={m.title} style={{ padding: "18px 20px", borderRadius: 14, background: "rgba(255,255,255,0.02)", border: `1px solid ${m.color}25`, display: "flex", alignItems: "center", gap: 16 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 4 }}>{m.title}</div>
                      <Badge style={{ background: "rgba(255,255,255,0.04)", color: "var(--comic-text-secondary, #6B7280)", border: "1px solid rgba(255,255,255,0.06)", fontSize: 11 }}>{m.difficulty}</Badge>
                    </div>
              ...
              <div style={{ fontSize: 20, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 12 }}>Where to Spend</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }}>
                {SPEND_OPTIONS.map((s) => (
                  <div key={s.title} style={{ padding: "16px", borderRadius: 14, background: `${s.color}08`, border: `1px solid ${s.color}20`, cursor: "pointer" }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 700, color: "var(--comic-text-primary, #E8EAF0)", marginBottom: 4 }}>{s.title}</div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ padding: "8px 24px 20px", flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14 }}>
                ...
                <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="Ask about earning, spending, or transferring credits…" style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontSize: 14, color: "var(--comic-text-primary, #E8EAF0)" }} />
      ...
      <aside style={{ width: 280, borderLeft: "1px solid rgba(255,255,255,0.06)", background: "var(--comic-surface, #0D0F14)", padding: "20px 16px", flexShrink: 0 }}>
        ...
        <div style={{ padding: "16px", borderRadius: 14, background: `${COLOR}08`, border: `1px solid ${COLOR}20`, marginBottom: 16 }}>
          <input placeholder="Survivor username or ID…" style={{ width: "100%", padding: "10px 12px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, fontSize: 13, color: "var(--comic-text-primary, #E8EAF0)", outline: "none", marginBottom: 8, boxSizing: "border-box" }} />
          <input value={sendAmount} onChange={(e) => setSendAmount(e.target.value)} placeholder="Amount (e.g. 50)" style={{ width: "100%", padding: "10px 12px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, fontSize: 13, color: "var(--comic-text-primary, #E8EAF0)", outline: "none", marginBottom: 10, boxSizing: "border-box" }} />
          <button style={{ width: "100%", padding: "10px", borderRadius: 10, background: COLOR, border: "none", color: "var(--comic-bg, #0F1117)", fontSize: 14, fontWeight: 800, cursor: "pointer" }}>Send Credits</button>
        </div>
        ...
        <div style={{ marginTop: 16, padding: "14px 16px", borderRadius: 12, background: `${COLOR}06`, border: `1px solid ${COLOR}18` }}>
          ...
          <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)", lineHeight: 1.6 }}>Every transaction is recorded on the Formance open-source ledger. Transparent, immutable, and verifiable.</div>
        </div>
      </aside>
    </div>
  );
