    ...
    return (
      <div style={{ width: 390, height: "100%", minHeight: "100vh", background: "var(--comic-bg, #0F1117)", fontFamily: "'Inter', system-ui, sans-serif", color: "var(--comic-text-primary, #E8EAF0)", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "14px 20px", background: "var(--comic-surface-alt, #090B0F)", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1, fontSize: 16, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)", textAlign: "center" }}>Provider</div>
          <div style={{ width: 40 }} />
        </div>
        <ScrollArea style={{ flex: 1 }}>
          <div style={{ padding: "20px 20px" }}>
            <div style={{ textAlign: "center", marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 4 }}>{p.name}</div>
              ...
              <div style={{ display: "flex", gap: 6, justifyContent: "center", flexWrap: "wrap" }}>
                ...
                <Badge style={{ background: p.available ? "#22C55E20" : "rgba(255,255,255,0.05)", color: p.available ? "#22C55E" : "var(--comic-text-secondary, #6B7280)", border: `1px solid ${p.available ? "#22C55E40" : "rgba(255,255,255,0.08)"}`, fontSize: 11 }}>{p.available ? "● Available" : "Busy"}</Badge>
            ...
            <div style={{ padding: "14px", borderRadius: 12, background: `${COLOR}08`, border: `1px solid ${COLOR}18` }}>
              ...
              <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)", lineHeight: 1.6 }}>Background-checked, insured, trauma-informed. All communications are encrypted.</div>
            </div>
          </div>
        </ScrollArea>
      </div>
    ...
    <div style={{ width: 390, height: "100%", minHeight: "100vh", background: "var(--comic-bg, #0F1117)", fontFamily: "'Inter', system-ui, sans-serif", color: "var(--comic-text-primary, #E8EAF0)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 20px 12px", background: "var(--comic-surface-alt, #090B0F)", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)" }}>Foundation</div>
            ...
            <div key={p.id} onClick={() => setSelected(p.id)} style={{ padding: "14px", borderRadius: 14, background: "rgba(255,255,255,0.02)", border: `1px solid ${COLOR}15`, marginBottom: 10, cursor: "pointer", display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: "var(--comic-text-primary, #F9FAFB)" }}>{p.name}</div>
