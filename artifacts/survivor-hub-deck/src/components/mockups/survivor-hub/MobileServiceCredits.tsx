    ...
    <div style={{ width: 390, height: "100%", minHeight: "100vh", background: "var(--comic-bg, #0F1117)", fontFamily: "'Inter', system-ui, sans-serif", color: "var(--comic-text-primary, #E8EAF0)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 20px 12px", background: "var(--comic-surface-alt, #090B0F)", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)" }}>Service Credits</div>
        ...
        <div style={{ padding: "6px 12px", borderRadius: 10, background: `${COLOR}08`, border: `1px solid ${COLOR}20`, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 9, color: "var(--comic-text-secondary, #6B7280)" }}>credits</div>
        </div>
      </div>
      <ScrollArea style={{ flex: 1 }}>
        <div style={{ padding: "16px" }}>
            ...
            <>
              ...
              <div style={{ padding: "20px", borderRadius: 16, background: `linear-gradient(135deg,${COLOR}25 0%,rgba(245,158,11,0.05) 100%)`, border: `1px solid ${COLOR}30`, marginBottom: 16, textAlign: "center" }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button style={{ flex: 1, padding: "10px", borderRadius: 10, background: COLOR, border: "none", color: "var(--comic-bg, #0F1117)", fontSize: 13, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}><ArrowUp size={14} /> Send</button>
              ...
              <div style={{ padding: "14px", borderRadius: 14, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 12 }}>Recent Transactions</div>
                {TRANSACTIONS.map((t) => (
                  <div key={t.id} style={{ display: "flex", gap: 10, alignItems: "center", padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 600, color: "var(--comic-text-primary, #E8EAF0)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{t.description}</div>
          ...
          {activeNav === "earn" && (
            <>
              <div style={{ fontSize: 16, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 4 }}>Earn Credits</div>
              <div style={{ fontSize: 12, color: "var(--comic-text-secondary, #6B7280)", marginBottom: 14 }}>Contribute to the community and get rewarded</div>
              {EARN_WAYS.map((w) => (
                <div key={w.title} style={{ padding: "14px", borderRadius: 14, background: "rgba(255,255,255,0.02)", border: `1px solid ${w.color}25`, marginBottom: 10, display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 2 }}>{w.title}</div>
                  </div>
              ...
              <div style={{ padding: "14px", borderRadius: 12, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", marginTop: 8 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 8 }}>Spend Everywhere</div>
          ...
          {activeNav === "send" && (
            <div>
              <div style={{ fontSize: 16, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 16 }}>Send Credits</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <input placeholder="Survivor username or ID…" style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, fontSize: 14, color: "var(--comic-text-primary, #E8EAF0)", outline: "none", boxSizing: "border-box" }} />
                <input value={sendAmount} onChange={(e) => setSendAmount(e.target.value)} placeholder="Amount (e.g. 50)" style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, fontSize: 14, color: "var(--comic-text-primary, #E8EAF0)", outline: "none", boxSizing: "border-box" }} />
                <textarea placeholder="Note (optional)" rows={2} style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, fontSize: 14, color: "var(--comic-text-primary, #E8EAF0)", outline: "none", resize: "none", boxSizing: "border-box" }} />
                <button style={{ width: "100%", padding: "14px", borderRadius: 14, background: COLOR, border: "none", color: "var(--comic-bg, #0F1117)", fontSize: 15, fontWeight: 800, cursor: "pointer" }}>Send {sendAmount || "0"} Credits</button>
                <div style={{ padding: "12px 14px", borderRadius: 12, background: `${COLOR}06`, border: `1px solid ${COLOR}18` }}>
                  ...
                  <div style={{ fontSize: 11, color: "var(--comic-text-secondary, #6B7280)" }}>Every transaction recorded on the open-source Formance ledger. Transparent and immutable.</div>
                </div>
              </div>
            </div>
          )}
          {activeNav === "home" && (
            <div style={{ textAlign: "center", padding: "32px 0" }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 800, color: "var(--comic-text-primary, #F9FAFB)", marginBottom: 6 }}>Service Credits</div>
              <div style={{ fontSize: 13, color: "var(--comic-text-secondary, #6B7280)", marginBottom: 20 }}>Earn, spend, trade across all 12 apps</div>
              <div style={{ fontSize: 36, fontWeight: 900, color: COLOR }}>2,420</div>
              <div style={{ fontSize: 13, color: "var(--comic-text-secondary, #6B7280)" }}>your balance</div>
            </div>
          )}
        </div>
      </ScrollArea>
